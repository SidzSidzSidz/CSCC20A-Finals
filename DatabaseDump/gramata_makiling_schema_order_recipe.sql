...
CREATE TABLE `order_recipe` (
  ...
  PRIMARY KEY (`onum`,`recipe_num`),
  KEY `recipe_num_idx` (`recipe_num`),
  KEY `recipe_num_iddsas` (`recipe_num`),
  CONSTRAINT `onum_pkthree` FOREIGN KEY (`onum`) REFERENCES `orders` (`onum`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `recipe_num_pkthree` FOREIGN KEY (`recipe_num`) REFERENCES `recipe` (`recipe_num`) ON DELETE CASCADE ON UPDATE CASCADE
